	...
	<div>
		<div id="commentInfo">
			<router-link
				...
				:to="{
					name: 'mypage',
					params: { no: commentInfo.comment.memberId }
				}"
			>
				...
				<img
					id="commentWriterImg"
					:src="
						commentInfo.postWriterProfileImage == ''
							? '../img/icons/user.png'
							: commentInfo.postWriterProfileImage
					"
					alt="../assets/user.png"
				/>
				<p id="commentWriterNickname">
					{{ commentInfo.comment.commentWriter }}
				</p>
			</router-link>
			<v-icon
				id="commentEdit"
			...
			>
			<div id="commentDate" class="ml-auto">
				{{ commentInfo.comment.updateCreated }}
			</div>
		</div>

		<div id="commentContent" v-show="!show">
			{{ commentInfo.comment.contents }}
		</div>
		<v-card-text
			:id="'commentUpdateExpand-' + commentInfo.comment.idcomment"
			v-show="show"
		>
			<textarea
				:id="'commentUpdateInput-' + commentInfo.comment.idcomment"
				v-model="contents"
			/>
...
export default {
	...
	methods: {
		commentEditForm() {
			...
			document
				.getElementById(
					`commentUpdateInput-${this.commentInfo.comment.idcomment}`
				)
				.focus();
		},
		commentEdit() {
